...
This returns: current branch, last commits, uncommitted changes, rebase status, and open PR info.
...
- If the branch has an open PR, read the PR description and comments
- Verify build with `constellation_get_build_status` or `make compile`

### Pre-Work Conflict Check

Before modifying files, check if other agents have open PRs touching the same files:

```powershell
# Using the conflict check script
.\scripts\check-conflicts.ps1 -Files "path/to/file1.scala","path/to/file2.scala"
...
If conflicts are detected, coordinate with the user before proceeding.
