...
4. **Side effects are final.** Once a module fires, its result is captured in the suspended state and the module is never re-executed on resume.
...
trait SuspensionStore {
  ...
  def save(signature: DataSignature): IO[SuspensionHandle]

  /** Load a suspended execution by handle. */
  def load(handle: SuspensionHandle): IO[Option[DataSignature]]

  /** Resume a suspended execution with additional inputs.
    * Consumer must provide the synthetic modules from their LoadedProgram.
    */
  def resumeFromStore(
...
val handle = constellation.runAndSuspend("onboarding", partialInputs, loaded.syntheticModules).unsafeRunSync()
val sig    = constellation.resumeFromStore(handle, moreInputs, loaded.syntheticModules).unsafeRunSync()
```
...
- `ExecutionOptions` with `includeTimings`, `includeProvenance`, `includeBlockedGraph`
- `SignatureMetadata` optional fields
- Per-node timing instrumentation
- Dependency trace collection
- Blocked graph computation
- Tests: each flag independently, all flags combined, performance comparison flags off vs on
...
- `Constellation.builder` integration
- Convenience methods: `runAndSuspend`, `resumeFromStore`
- Tests: store/load/resume round-trip, list/filter, delete, concurrent access
