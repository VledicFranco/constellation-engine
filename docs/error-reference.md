...
**Resolution:**
- Check the [constellation-lang documentation](constellation-lang/README.md) for supported operations
...
**Resolution:**
- Use a key with the appropriate role (see [Security Model](security.md) for role permissions)
...
Parse and compile errors from the constellation-lang compiler use a separate error hierarchy (`CompileError`) with source position information. These are documented in detail in the [Error Messages Guide](constellation-lang/error-messages.md).
