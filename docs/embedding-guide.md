...
| Entry Point | Purpose |
|-------------|---------|
| `Constellation` trait | Register modules, store DAGs, execute pipelines |
| `LangCompiler` | Compile constellation-lang source into `DagSpec` |
...
object MinimalExample extends IOApp.Simple {
  ...
  def run: IO[Unit] =
    for {
      ...
      state <- constellation.runDagWithModules(
        compiled.dagSpec,
        inputs = Map("text" -> io.constellation.TypeSystem.CValue.VString("hello world")),
        modules = compiled.syntheticModules
      )

      // 6. Read outputs
      _ <- IO.println(s"Outputs: ${state.outputs}")
    } yield ()
}
```
...
`LangCompiler.compile` parses, type-checks, and compiles constellation-lang source into a `CompileResult`:

```scala
val result: Either[List[CompileError], CompileResult] =
...
|-------|------|-------------|
| `dagSpec` | `DagSpec` | The compiled DAG specification |
| `syntheticModules` | `Map[UUID, Module.Uninitialized]` | Modules generated by the compiler for merge, projection, and conditional operations |

### Execution

Pass the compiled DAG, inputs, and synthetic modules to the runtime:

```scala
val state: IO[Runtime.State] = constellation.runDagWithModules(
  compiled.dagSpec,
...
| Field | Description |
|-------|-------------|
| `outputs` | `Map[String, CValue]` — pipeline output values |
| `moduleStatuses` | Per-module execution status (Fired, Failed, Timed) |
| `data` | All intermediate data values |

### Reusing Compiled DAGs

Compile once, execute many times:

```scala
// Store the DAG for later execution by name
constellation.setDag("my-pipeline", compiled.dagSpec)

// Execute by name (modules must be pre-registered)
val state = constellation.runDag("my-pipeline", inputs)
```
...
object TextPipeline extends IOApp.Simple {
  ...
  def run: IO[Unit] =
    for {
      ...
      _ <- IO.println(s"upper = ${state.outputs.get("upper")}")
      _ <- IO.println(s"words = ${state.outputs.get("words")}")
    } yield ()
}
```
...
val execution: IO[CancellableExecution] =
  constellation.runDagCancellable("my-pipeline", inputs)
...
for {
  ...
  _      <- exec.cancel
  status <- exec.status  // ExecutionStatus.Cancelled
} yield ()
```
