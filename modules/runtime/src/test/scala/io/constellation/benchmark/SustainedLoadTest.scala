...
/** Sustained load tests (RFC-013 Phase 5.3)
  *
  * Runs 10K+ executions and verifies no OOM, bounded heap growth, and stable p99 latency over time.
  *
...
class SustainedLoadTest extends AnyFlatSpec with Matchers {
  ...
  "Sustained load" should "complete 10000 executions without OOM" in {
    ...
    (1 to totalRuns).foreach { i =>
      ...
      if i % 1000 == 0 then {
        ...
        println(
          f"  [$i%5d/$totalRuns] p50=${percentile(latencies, 50)}%.2f ms, p99=${percentile(latencies, 99)}%.2f ms, heap=${usedMB}%.0f MB"
        )
      }
    }
    ...
    println(
      f"Completed $totalRuns executions. Final p50=${percentile(latencies, 50)}%.2f ms, p99=${percentile(latencies, 99)}%.2f ms"
    )
  }
  ...
  private def percentile(data: Iterable[Double], p: Int): Double = {
    ...
    if sorted.isEmpty then return 0.0
