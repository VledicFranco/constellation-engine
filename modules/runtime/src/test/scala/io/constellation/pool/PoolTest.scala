...
class PoolTest extends AnyFlatSpec with Matchers {
  ...
  it should "clear state on release" in {
    val test = for {
      ...
      state <- pool.acquire
      // Modify state
      _ = state.moduleStatus.put(java.util.UUID.randomUUID(), cats.Eval.now(io.constellation.Module.Status.Unfired))
      _ = state.data.put(java.util.UUID.randomUUID(), cats.Eval.now(io.constellation.CValue.CInt(42)))
      _ <- pool.release(state)
  ...
  it should "provide combined metrics" in {
    val test = for {
      pool <- RuntimePool.create(RuntimePool.Config(
