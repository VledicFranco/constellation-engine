...
class GlobalSchedulerTest extends AnyFlatSpec with Matchers {
  ...
  it should "handle errors properly" in {
    val error = intercept[RuntimeException] {
      GlobalScheduler.unbounded.submit(50, IO.raiseError[Int](new RuntimeException("Test error"))).unsafeRunSync()
    }
  ...
  "Bounded scheduler priority ordering" should "execute higher priority tasks before lower priority" in {
    GlobalScheduler.bounded(maxConcurrency = 1).use { scheduler =>
      ...
      for {
        ...
        lowFiber <- scheduler.submit(20, IO {
        ...
        highFiber <- scheduler.submit(80, IO {
  ...
  "Bounded scheduler starvation prevention" should "boost priority of waiting tasks over time" in {
    ...
    GlobalScheduler.bounded(maxConcurrency = 1, starvationTimeout = 1.second).use { scheduler =>
      for {
  ...
  "Bounded scheduler shutdown" should "reject new tasks after shutdown" in {
    val (scheduler, shutdown) = GlobalScheduler.bounded(maxConcurrency = 2)
      .allocated
      .unsafeRunSync()
  ...
  "Bounded scheduler error handling" should "propagate task errors" in {
    GlobalScheduler.bounded(maxConcurrency = 2).use { scheduler =>
      val error = intercept[RuntimeException] {
        scheduler.submit(50, IO.raiseError[Int](new RuntimeException("Task failed"))).unsafeRunSync()
      }
  ...
  it should "continue processing after task error" in {
    GlobalScheduler.bounded(maxConcurrency = 2).use { scheduler =>
      for {
        // First task fails
        _ <- scheduler.submit(50, IO.raiseError[Int](new RuntimeException("Error")))
          .attempt
  ...
  "Bounded scheduler concurrent submissions" should "handle many concurrent submitters" in {
    GlobalScheduler.bounded(maxConcurrency = 4).use { scheduler =>
  ...
  "QueueEntry" should "order by effective priority descending, then by id ascending" in {
    ...
    val highPriority = QueueEntry(id = 1, priority = 80, submittedAt = now, gate = null, effectivePriority = 80)
    val lowPriority = QueueEntry(id = 2, priority = 20, submittedAt = now, gate = null, effectivePriority = 20)
    val medPriority = QueueEntry(id = 3, priority = 50, submittedAt = now, gate = null, effectivePriority = 50)
  ...
  it should "use FIFO order for same priority" in {
    ...
    val first = QueueEntry(id = 1, priority = 50, submittedAt = now, gate = null, effectivePriority = 50)
    val second = QueueEntry(id = 2, priority = 50, submittedAt = now, gate = null, effectivePriority = 50)
    val third = QueueEntry(id = 3, priority = 50, submittedAt = now, gate = null, effectivePriority = 50)
  ...
  "SchedulerState" should "enqueue and dequeue correctly" in {
    ...
    val entry1 = QueueEntry(id = 1, priority = 50, submittedAt = 0.seconds, gate = null, effectivePriority = 50)
    val entry2 = QueueEntry(id = 2, priority = 80, submittedAt = 0.seconds, gate = null, effectivePriority = 80)
