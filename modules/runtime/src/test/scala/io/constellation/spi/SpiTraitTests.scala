...
class SpiTraitTests extends AnyFlatSpec with Matchers {
  ...
  it should "return IO.unit for histogram with tags" in {
    MetricsProvider.noop.histogram("test.histogram", 42.0, Map("key" -> "value")).unsafeRunSync() shouldBe ()
  }
  ...
  it should "pass through body with attributes" in {
    val result = TracerProvider.noop.span("test.span", Map("key" -> "value"))(IO.pure("hello")).unsafeRunSync()
    result shouldBe "hello"
  }

  it should "propagate errors from body" in {
    val error = new RuntimeException("test error")
    val result = TracerProvider.noop.span("test.span")(IO.raiseError[Int](error)).attempt.unsafeRunSync()
  ...
  "ExecutionListener.noop" should "return IO.unit for all callbacks" in {
    ...
    ExecutionListener.noop.onModuleComplete(id, moduleId, "TestModule", 100L).unsafeRunSync() shouldBe ()
    ExecutionListener.noop.onModuleFailed(id, moduleId, "TestModule", new RuntimeException("fail")).unsafeRunSync() shouldBe ()
    ExecutionListener.noop.onExecutionComplete(id, "testDag", true, 500L).unsafeRunSync() shouldBe ()
  }
