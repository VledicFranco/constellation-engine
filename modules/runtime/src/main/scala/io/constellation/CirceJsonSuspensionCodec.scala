...
/** Circe JSON implementation of [[SuspensionCodec]].
  *
  * Serializes [[SuspendedExecution]] to/from JSON bytes using the codecs
  * defined in [[CustomJsonCodecs]].
  */
object CirceJsonSuspensionCodec extends SuspensionCodec with CustomJsonCodecs {
  ...
  def encode(suspended: SuspendedExecution): Either[CodecError, Array[Byte]] =
    try {
      ...
      case e: Exception => Left(CodecError(s"Failed to encode SuspendedExecution: ${e.getMessage}", Some(e)))
    }
    ...
    parse(jsonStr) match {
      ...
      case Right(json) =>
        json.as[SuspendedExecution](using suspendedExecutionDecoder) match {
          case Left(e)  => Left(CodecError(s"Failed to decode SuspendedExecution: ${e.getMessage}", Some(e)))
