...
final class LazyListValue(
  ...
  def get(index: Int): Either[String, CValue] =
    if index < 0 || index >= jsonArray.size then {
...
object LazyListValue {
  ...
  def fromJson(json: Json, elementType: CType): Either[String, LazyListValue] =
    json.asArray match {
      ...
      case None      => Left(s"Expected array, got ${if json.isNull then "null" else json.name}")
    }
}
    ...
    fieldTypes.get(name) match {
      ...
      case Some(fieldType) =>
        jsonObj.get(name) match {
          ...
          case Some(json) =>
            cache
              .getOrElseUpdate(name, JsonCValueConverter.jsonToCValue(json, fieldType, name))
              .map(Some(_))
        }
    }
...
object LazyProductValue {
  ...
  def fromJson(json: Json, fieldTypes: Map[String, CType]): Either[String, LazyProductValue] =
    json.asObject match {
      ...
      case None      => Left(s"Expected object, got ${if json.isNull then "null" else json.name}")
    }
}
