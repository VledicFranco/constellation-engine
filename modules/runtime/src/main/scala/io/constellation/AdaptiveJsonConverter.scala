...
/** Adaptive JSON converter that chooses the best strategy based on payload characteristics.
  ...
  * ==Strategies==
  *
  * - **Eager** (current): Best for small payloads (<10KB). Uses recursive descent with
  *   full materialization. Lowest overhead for small data.
  *
  * - **Lazy**: Best for medium payloads (10KB-100KB) or when partial access is expected.
  *   Wraps JSON and defers conversion until values are accessed.
  *
  * - **Streaming**: Best for large payloads (>100KB). Uses Jackson streaming parser
  *   for memory-efficient parsing without loading entire structure.
  *
  * ==Usage==
  *
  * {{{
...
class AdaptiveJsonConverter(
  ...
  /** Convert JSON to CValue using the optimal strategy.
    ...
    * @param sizeHint Optional size hint in bytes (avoids re-estimation)
  ...
  /** Convert using streaming strategy.
    * Serializes JSON to bytes, then parses with Jackson streaming.
    */
  ...
  /** Convert using lazy strategy.
    * Creates lazy wrappers that defer conversion.
    */
  ...
  /** Estimate JSON size in bytes without full serialization.
    * Uses structural heuristics for speed.
    */
    ...
    if (depth > 50) {
      ...
      json.fold(
        ...
        jsonNumber = n => {
          ...
          val s = n.toString
          if (s.contains(".")) s.length + 1 else s.length
        },
        ...
        jsonObject = obj => {
          if (obj.isEmpty) 2
            ...
            val avgFieldSize = if (sample.nonEmpty) {
              sample.map { case (k, v) =>
