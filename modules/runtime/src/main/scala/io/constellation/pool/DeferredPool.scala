 ...
 *
 * Deferreds in Cats Effect are one-shot: once completed, they cannot be reset.
 * This pool manages a supply of fresh, uncompleted Deferreds that can be acquired
 * for DAG execution. After execution completes, the pool replenishes itself with
 * fresh Deferreds for subsequent requests.
 *
 * == Thread Safety ==
 *
 ...
 * == Memory Model ==
 *
 * The pool maintains between `initialSize` and `maxSize` Deferreds. When depleted,
 * it creates fresh Deferreds on demand. After execution, calling `replenish`
 * restores the pool to its target size.
 *
 * @param initialSize Number of Deferreds to pre-allocate on initialization
...
final class DeferredPool private (
  ...
  /**
   * Acquire a single Deferred from the pool.
   * If the pool is empty, creates a fresh Deferred.
   */
   ...
   * Takes as many as available from the pool, creates the rest.
   */
  def acquireN(n: Int): IO[List[Deferred[IO, Any]]] = {
    if (n <= 0) IO.pure(Nil)
    else {
      pool.modify { available =>
   ...
   *
   * Since completed Deferreds cannot be reused, this creates fresh ones
   * up to the target size (min of count and available capacity).
   */
