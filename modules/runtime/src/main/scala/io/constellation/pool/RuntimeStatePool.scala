...
final class RuntimeStatePool private (
  ...
  /** Acquire a state container from the pool. Creates a new one if the pool is empty.
    */
  ...
  /** Release a state container back to the pool. The container is cleared before being added to the
    * pool.
    */
...
object RuntimeStatePool {
  ...
  /** Mutable state container that can be reused between executions.
    *
    * Uses mutable collections internally for performance, but access is controlled through the
    * pool's acquire/release mechanism.
    */
  final class PooledState private (
    ...
    /** Reset the state for reuse. Called when releasing back to the pool.
      */
    def reset(): Unit = {
      moduleStatus.clear()
      data.clear()
