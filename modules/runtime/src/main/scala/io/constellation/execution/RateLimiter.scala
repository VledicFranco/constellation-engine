...
/** Rate limit specification.
  *
  * @param count Maximum number of operations allowed
  * @param perDuration Time window for the rate limit
  */
...
/** Token bucket rate limiter.
  ...
  * Multiple fibers can safely call acquire concurrently.
  */
class TokenBucketRateLimiter private (
  ...
  /** Acquire a token, waiting if necessary.
    ...
    * 3. If no tokens, wait for the next token and retry
    *
    * @return IO that completes when a token is acquired
    */
  def acquire: IO[Unit] = {
    for {
      ...
      _ <- refillTokens(now)
      acquired <- tryAcquire
      _ <- if (acquired) IO.unit else waitAndRetry
    } yield ()
  }

  /** Try to acquire a token without waiting.
    *
    * @return true if token was acquired, false if no tokens available
    */
  ...
  /** Execute an operation with rate limiting.
    *
    * Acquires a token before executing, ensuring the operation
  ...
  private def refillTokens(now: Long): IO[Unit] = {
    lastRefillRef.modify { lastRefill =>
...
object TokenBucketRateLimiter {
  ...
  /** Create a new rate limiter.
    *
    * @param rate The rate limit to enforce
    * @return A new token bucket rate limiter
    */
  ...
  /** Create a rate limiter with initial token count.
    ...
    *
    * @param rate The rate limit to enforce
    * @param initialTokens Initial number of tokens (default: full bucket)
    */
