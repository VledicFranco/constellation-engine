...
object BackoffStrategy {
  ...
  def computeDelay(
    ...
    if computed > maxDelay then maxDelay else computed
  }
}
...
object ModuleExecutor {
  ...
  /** Execute with retry logic.
    *
    * Retries the operation up to `maxRetries` times on failure. Each attempt is independent -
    * errors from previous attempts don't affect subsequent ones.
    *
    * @param operation
    ...
    * @param backoff
    *   Backoff strategy for delay calculation (default: Fixed)
    * @param maxDelay
    *   Maximum delay cap for backoff strategies
    * @param onRetry
    *   Optional callback invoked before each retry with attempt number and error
    * @return
    *   The result of the first successful attempt, or the last error if all fail
    */
  def executeWithRetry[A](
      ...
      operation.handleErrorWith { error =>
        ...
        if remaining > 0 then {
          ...
          val delayEffect = delay match {
            case Some(baseDelay) =>
              val computedDelay =
                BackoffStrategy.computeDelay(baseDelay, attemptNum, backoff, maxDelay)
    ...
    if maxRetries <= 0 then operation
  ...
  /** Execute with timeout.
    ...
    * @return
    *   The result if completed within timeout, or ModuleTimeoutException
    */
  def executeWithTimeout[A](
      operation: IO[A],
      timeout: FiniteDuration
  ): IO[A] =
    operation.timeout(timeout).adaptError { case _: java.util.concurrent.TimeoutException =>
      ModuleTimeoutException(s"Operation timed out after ${timeout.toMillis}ms", timeout)
    }

  /** Execute with fallback.
    *
    * Returns the fallback value if the operation fails. The fallback is only evaluated if needed
    * (lazy evaluation).
    *
    * @param operation
    *   The IO operation to execute
    * @param fallback
    *   The fallback value to use on failure (lazy)
    * @param onFallback
    *   Optional callback invoked when fallback is used with the original error
    * @return
    *   The result of the operation, or the fallback value on failure
    */
  ...
  /** Combined execution with all options.
    ...
    *   1. Timeout (per attempt) 2. Retry (wraps timeout) 3. Fallback (wraps retry)
    *
    * @param operation
    *   The IO operation to execute
    * @param options
    *   Execution options
    * @return
    *   The result according to the configured options
    */
  def execute[A](
      operation: IO[A],
...
final case class RetryExhaustedException(
  ...
  def detailedMessage: String = {
    val errorDetails = errors.zipWithIndex
      .map { case (e, i) =>
