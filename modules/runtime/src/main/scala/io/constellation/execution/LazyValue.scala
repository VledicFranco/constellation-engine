...
/** A lazy, memoized value.
  *
  * The computation is deferred until `force` is called. Once computed,
  * the result is cached and subsequent `force` calls return immediately.
  *
  * ==Thread Safety==
  *
  * This implementation is thread-safe. If multiple fibers call `force`
  * concurrently, only one computation runs and others wait for the result.
  *
  * ==Usage==
  *
  * {{{
  ...
  * ==Memory==
  *
  * The cached result is held in memory until the LazyValue is garbage collected.
  * For large results that should be recomputed, consider using a cache with TTL.
  */
class LazyValue[A] private (
        ...
        Deferred[IO, A].flatMap { deferred =>
          stateRef.modify {
            case LazyValue.Pending =>
              ...
              val computation = compute.flatTap { result =>
  ...
  /** Check if the value has been computed.
    *
    * Note: This is a snapshot in time. The value may be computed
  ...
  /** Reset to pending state, allowing recomputation.
    *
    * Use with caution - this discards any cached result.
    * If computation is in progress, waits for it to complete first.
    */
  def reset: IO[Unit] = {
    stateRef.get.flatMap {
      case LazyValue.Computing(deferred) =>
  ...
  /** Map over the lazy value.
    *
    * Creates a new LazyValue that applies `f` to the result.
    * The original computation is shared - if this value is forced,
    * the mapped value doesn't recompute.
    */
...
object LazyValue {
  ...
  /** Create a new lazy value.
    *
    * @param compute The computation to defer
    * @return A lazy value wrapping the computation
    */
