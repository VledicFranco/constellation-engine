...
/** Describes the outcome of executing a pipeline.
  *
  * Contains the execution status, all computed values, declared outputs,
  * information about missing inputs (for suspended executions), and optional
  * metadata. When the status is [[PipelineStatus.Suspended]] or
  * [[PipelineStatus.Failed]], a [[SuspendedExecution]] snapshot is included
  * so the execution can be resumed.
  *
  * @param executionId     Unique ID for this execution run
  * @param structuralHash  Structural hash of the program that was executed
  * @param resumptionCount Number of times this execution has been resumed (0 for first run)
  * @param status          Terminal status of the execution
  * @param inputs          Input values that were provided
  * @param computedNodes   All computed data node values (keyed by variable name)
  * @param outputs         Subset of computedNodes matching declared outputs
  * @param missingInputs   Input variable names that were expected but not provided
  * @param pendingOutputs  Declared output names that were not computed
  * @param suspendedState  Snapshot for resumption (present when Suspended or Failed)
  * @param metadata        Optional timing/provenance metadata
  */
final case class DataSignature(
  ...
  def progress: Double = {
    ...
    if (total == 0) 1.0 else outputs.size.toDouble / total
  }
