...
/**
 * Stepped execution engine for debugging DAG pipelines.
 * Allows batch-by-batch execution with state inspection between steps.
 */
object SteppedExecution {
  ...
  /**
   * Compute execution batches using Kahn's algorithm for topological ordering.
   * Groups modules by dependency level - modules in the same level can run in parallel.
   */
  def computeBatches(dagSpec: DagSpec): List[ExecutionBatch] = {
    ...
    while (remaining.nonEmpty) {
      ...
      if (readyModules.isEmpty && remaining.nonEmpty) {
        throw new RuntimeException("Cycle detected in DAG")
      }

      if (readyModules.nonEmpty) {
  ...
  def initializeRuntime(session: SessionState): IO[SessionState] = {
    ...
    for {
      ...
      topLevelNodeStates = dag.topLevelDataNodes.map { case (dataId, spec) =>
        ...
        val value = session.inputs.collectFirst {
  ...
  /**
   * Execute the next batch of modules.
   * Returns the updated session state and whether execution is complete.
   */
  ...
  def executeToCompletion(session: SessionState): IO[SessionState] = {
    def loop(currentSession: SessionState): IO[SessionState] = {
      if (currentSession.currentBatchIndex >= currentSession.batches.length) {
  ...
  def getOutputs(session: SessionState): Map[String, CValue] = {
    ...
    declaredOutputs.flatMap { outputName =>
      outputBindings.get(outputName).flatMap { dataNodeUuid =>
        session.nodeStates.get(dataNodeUuid).collect {
          case NodeState.Completed(value, _) => outputName -> value
        }
      }
    }.toMap
  }
  ...
  def valuePreview(value: CValue, maxLength: Int = 50): String = {
    ...
    if (str.length > maxLength) str.take(maxLength - 3) + "..." else str
  }
  ...
  private def initState(dag: DagSpec): IO[Ref[IO, Runtime.State]] = {
    ...
    Ref.of[IO, Runtime.State](Runtime.State(
  ...
  private def completeTopLevelDataNodes(dag: DagSpec, initData: Map[String, CValue], rt: Runtime): IO[Unit] =
