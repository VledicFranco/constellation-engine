...
/** Pluggable cache backend interface.
  ...
  * All operations return IO to allow for async backends (e.g., Redis).
  *
  * ==Usage==
  *
  * {{{
...
trait CacheBackend {
  ...
  /** Get a value from the cache.
    ...
    * @return Some(entry) if found and not expired, None otherwise
    */
  ...
  /** Get or compute a value.
    ...
    *
    * @param key The cache key
    * @param ttl Time-to-live for newly computed values
    * @param compute The computation to run on cache miss
