...
/** Generates deterministic cache keys from module names and inputs.
  ...
  * }}}
  *
  * The canonical serialization ensures that equivalent inputs
  * produce the same key, regardless of map ordering, etc.
  */
object CacheKeyGenerator {
  ...
  /** Generate a cache key for a module call.
    ...
    * @param inputs The input values to the module
    * @param version Optional version to invalidate cache on module changes
  ...
  /** Generate a short key (truncated hash) for display purposes.
    * Not suitable for production cache keys due to collision risk.
    */
  ...
  /** Create the canonical string representation for hashing.
    * Ensures deterministic ordering of map entries.
    */
