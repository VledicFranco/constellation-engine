...
object RawValueConverter {
  ...
  /**
   * Convert a CValue to its memory-efficient RawValue representation.
   * Type information is stripped and must be stored separately (in DataNodeSpec.cType).
   */
  ...
  /**
   * Convert a RawValue back to CValue using type information.
   * Delegates to TypedValueAccessor.
   */
