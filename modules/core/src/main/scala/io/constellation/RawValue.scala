...
sealed trait RawValue {
  ...
  /** Debug string representation that includes value info. For type information, use the CType from
    * the corresponding DataNodeSpec.
    */
...
object RawValue {
  ...
  /** Specialized list of unboxed integers. Memory: ~8 bytes per element (vs ~24 bytes with
    * CIntValue wrapper)
    */
  ...
  /** Specialized list of unboxed doubles. Memory: ~8 bytes per element (vs ~24 bytes with
    * CFloatValue wrapper)
    */
  ...
  final case class RStringList(values: Array[String]) extends RawValue {
    ...
    override def equals(other: Any): Boolean = other match {
      case that: RStringList =>
        java.util.Arrays
          .equals(this.values.asInstanceOf[Array[AnyRef]], that.values.asInstanceOf[Array[AnyRef]])
      case _ => false
    }
  ...
  /** Map from RawValue keys to RawValue values. Uses Array of tuples for better cache locality than
    * HashMap.
    */
  final case class RMap(entries: Array[(RawValue, RawValue)]) extends RawValue {
    ...
    def get(key: RawValue): Option[RawValue] = {
      ...
      while i < entries.length do {
        if entries(i)._1 == key then return Some(entries(i)._2)
        i += 1
      }
      None
    }
  ...
  /** Record with fields stored by index (field order from CType.CProduct). Field names are not
    * stored - they come from the associated CType.
    */
