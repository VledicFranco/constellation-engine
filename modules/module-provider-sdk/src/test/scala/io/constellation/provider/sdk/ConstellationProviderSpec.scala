...
class ConstellationProviderSpec extends AnyFlatSpec with Matchers {
  ...
  it should "create provider with discovery strategy" in {
    val provider = ConstellationProvider.withDiscovery(
      namespace = "test",
  ...
  it should "register multiple modules" in {
    ...
    val m1 = ModuleDefinition("upper", CType.CString, CType.CString, "1.0.0", "Upper", v => IO.pure(v))
    val m2 = ModuleDefinition("lower", CType.CString, CType.CString, "1.0.0", "Lower", v => IO.pure(v))
