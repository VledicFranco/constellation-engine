...
class CompilerRuntimeIntegrationTest extends AnyFlatSpec with Matchers {
  ...
  "filter with gt predicate" should "keep elements greater than threshold" in {
    ...
    """
    val inputs =
      Map("numbers" -> CValue.CList(Vector(1L, -2L, 3L, -4L, 5L).map(CValue.CInt), CType.CInt))
    val result = compileAndRun(source, inputs, "positives")

    result shouldBe CValue.CList(Vector(1L, 3L, 5L).map(CValue.CInt), CType.CInt)
  }
  ...
  it should "handle filter with lt predicate" in {
    ...
    result shouldBe CValue.CList(Vector(-2L, -4L).map(CValue.CInt), CType.CInt)
  }
  ...
  it should "handle filter with gte predicate" in {
    ...
    """
    val inputs =
      Map("numbers" -> CValue.CList(Vector(1L, -2L, 0L, 3L, -4L).map(CValue.CInt), CType.CInt))
    val result = compileAndRun(source, inputs, "nonNegatives")

    result shouldBe CValue.CList(Vector(1L, 0L, 3L).map(CValue.CInt), CType.CInt)
  }
  ...
  it should "handle filter with lte predicate" in {
    ...
    result shouldBe CValue.CList(Vector(-2L, 0L, -4L).map(CValue.CInt), CType.CInt)
  }
  ...
  it should "handle filter with eq-int predicate" in {
    ...
    """
    val inputs =
      Map("numbers" -> CValue.CList(Vector(1L, 0L, 2L, 0L, 3L).map(CValue.CInt), CType.CInt))
    val result = compileAndRun(source, inputs, "zeros")

    result shouldBe CValue.CList(Vector(0L, 0L).map(CValue.CInt), CType.CInt)
  }
  ...
  "map with arithmetic" should "double all elements" in {
    val source = """
      ...
      out doubled
    """
    val inputs = Map("numbers" -> CValue.CList(Vector(1L, 2L, 3L, 4L).map(CValue.CInt), CType.CInt))
    val result = compileAndRun(source, inputs, "doubled")

    result shouldBe CValue.CList(Vector(2L, 4L, 6L, 8L).map(CValue.CInt), CType.CInt)
  }
  ...
  it should "handle map with add operation" in {
    val source = """
      ...
      out incremented
    """
    val inputs = Map("numbers" -> CValue.CList(Vector(1L, 2L, 3L).map(CValue.CInt), CType.CInt))
    val result = compileAndRun(source, inputs, "incremented")

    result shouldBe CValue.CList(Vector(11L, 12L, 13L).map(CValue.CInt), CType.CInt)
  }
  ...
  it should "handle map with subtract operation" in {
    val source = """
      ...
      out decremented
    """
    val inputs = Map("numbers" -> CValue.CList(Vector(10L, 20L, 30L).map(CValue.CInt), CType.CInt))
    val result = compileAndRun(source, inputs, "decremented")

    result shouldBe CValue.CList(Vector(5L, 15L, 25L).map(CValue.CInt), CType.CInt)
  }
  ...
  it should "handle map with divide operation" in {
    ...
    result shouldBe CValue.CList(Vector(5L, 10L, 15L).map(CValue.CInt), CType.CInt)
  }
  ...
  it should "handle map with conditional lambda" in {
    ...
    """
    val inputs =
      Map("numbers" -> CValue.CList(Vector(1L, -2L, 3L, -4L).map(CValue.CInt), CType.CInt))
    val result = compileAndRun(source, inputs, "result")

    result shouldBe CValue.CList(Vector(1L, 0L, 3L, 0L).map(CValue.CInt), CType.CInt)
  }
  ...
  ignore should "combine multiple predicates with and" in {
    ...
    """
    val inputs =
      Map("numbers" -> CValue.CList(Vector(-5L, 1L, 5L, 15L, 20L).map(CValue.CInt), CType.CInt))
    val result = compileAndRun(source, inputs, "result")

    result shouldBe CValue.CList(Vector(1L, 5L).map(CValue.CInt), CType.CInt)
  }
  ...
  ignore should "accept elements matching either predicate with or" in {
    ...
    result shouldBe CValue.CList(Vector(-5L, 15L, 20L).map(CValue.CInt), CType.CInt)
  }
  ...
  ignore should "negate predicate result with not" in {
    ...
    """
    val inputs =
      Map("numbers" -> CValue.CList(Vector(0L, 1L, 0L, 2L, 3L).map(CValue.CInt), CType.CInt))
    val result = compileAndRun(source, inputs, "result")

    result shouldBe CValue.CList(Vector(1L, 2L, 3L).map(CValue.CInt), CType.CInt)
  }
  ...
  "map with nested conditional" should "handle complex logic" in {
    ...
    """
    val inputs =
      Map("numbers" -> CValue.CList(Vector(-5L, 0L, 5L, -3L, 0L, 10L).map(CValue.CInt), CType.CInt))
    val result = compileAndRun(source, inputs, "result")

    result shouldBe CValue.CList(Vector(-1L, 0L, 1L, -1L, 0L, 1L).map(CValue.CInt), CType.CInt)
  }
  ...
  ignore should "evaluate nested and/or/not" in {
    ...
    result shouldBe CValue.CList(Vector(-10L, 10L, -6L, 6L).map(CValue.CInt), CType.CInt)
  }
  ...
  "filter with empty list" should "return empty list" in {
    val source = """
      ...
      out result
    """
    val inputs = Map("numbers" -> CValue.CList(Vector.empty, CType.CInt))
    val result = compileAndRun(source, inputs, "result")
  ...
  "map with divide by zero" should "return 0 for division by zero" in {
    val source = """
      ...
      out result
    """
    val inputs = Map("numbers" -> CValue.CList(Vector(2L, 0L, 5L).map(CValue.CInt), CType.CInt))
    val result = compileAndRun(source, inputs, "result")

    result shouldBe CValue.CList(Vector(5L, 0L, 2L).map(CValue.CInt), CType.CInt)
  }
}
