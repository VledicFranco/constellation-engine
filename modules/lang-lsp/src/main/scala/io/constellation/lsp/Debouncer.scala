...
class Debouncer[K] private (
  ...
  /** Schedule an action to run after the delay, canceling any previously scheduled action for the
    * same key.
    *
    * @param key
    *   The key to debounce on (e.g., document URI)
    * @param action
  ...
  /** Execute an action immediately without debouncing, canceling any pending debounced action for
    * the same key.
    *
    * Use this for operations that should bypass debouncing (e.g., document save).
    *
    * @param key
    *   The key to run immediately
    * @param action
  ...
  def cancelAll: IO[Unit] =
    pending
      .modify(map => (Map.empty, map.values.toList))
      .flatMap(_.traverse_(_.cancel))

  /** Get the number of pending actions. Primarily useful for testing.
    */
  def pendingCount: IO[Int] =
    pending.get.map(_.size)

  /** Check if a key has a pending action. Primarily useful for testing.
    */
...
object Debouncer {
  ...
  /** Create a new Debouncer with the specified delay.
    *
    * @param delay
    *   How long to wait after the last call before executing
    * @tparam K
    *   The type of key used for debouncing
    */
  def create[K](delay: FiniteDuration = DefaultDelay): IO[Debouncer[K]] =
    Ref
      .of[IO, Map[K, Fiber[IO, Throwable, Unit]]](Map.empty)
