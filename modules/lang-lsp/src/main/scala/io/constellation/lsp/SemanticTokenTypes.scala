...
/** Semantic token types for LSP semantic highlighting.
  *
  * These types are used to classify tokens in the source code for
  * semantic-aware syntax highlighting. The indices must match the
  * order declared in the legend sent during initialization.
  *
...
object SemanticTokenTypes {
  ...
  /** Standard LSP semantic token types.
    * The index corresponds to the position in the legend's tokenTypes array.
    */
  ...
  /** Standard LSP semantic token modifiers.
    * These are represented as a bitmask - each modifier is a power of 2.
    */
  ...
  /** Token types as string list for LSP legend.
    * Order must match TokenType enum indices.
    */
  ...
  /** Token modifiers as string list for LSP legend.
    * Order must match bit positions in TokenModifier.
    */
